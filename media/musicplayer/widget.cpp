    ...
    : QMainWindow(parent)
{
    ...
    connect(mediaPlayList,SIGNAL(currentIndexChanged(int)),this,SLOT(mediaPlayerListCurrentIndexChanged(int)));
    connect(musicPlayer,SIGNAL(durationChanged(qint64)),this,SLOT(mediaPlayerDurationChanged(qint64)));
...
void MainWindow::musicLayout()
{
    ...
    QSpacerItem(320, 40,QSizePolicy::Minimum,QSizePolicy::Maximum);
...
void MainWindow::scanSongs()
{
    ...
    if (dirbsolutePath.exists()) {
        ...
        for (int i = 0; i < files.count(); i++) {
            ...
            /* 使用 utf-8 编码 */
            QString fileName = QString::fromUtf8(files.at(i).fileName().replace(".flac", "").toUtf8().data());
...
void MainWindow::mediaPlayerListCurrentIndexChanged(int index)
{
    if (-1 == index)
    return;
