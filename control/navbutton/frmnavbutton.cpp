...
void frmNavButton::initForm()
{
    ...
    //设置整体圆角 #2929290->rgb(41, 41, 41)->黑色
...
void frmNavButton::initBtn1()
{
    ...
    for(int i = 0; i < btns1.count(); i++){
        ...
        btn->setPaddingLeft(32);         //左边距间隔
        btn->setLineSpace(6);          //设置线条间隔

        btn->setShowIcon(true);          //设置显示图标
        btn->setIconSpace(15);           //设置图标间隔
        btn->setIconSize(QSize(10,10));  //设置图标尺寸

        btn->setIconNormal(iconNormal);  //设置正常图标
        btn->setIconHover(iconHover);    //设置悬停图标
        btn->setIconCheck(iconCheck);    //设置点击图标
