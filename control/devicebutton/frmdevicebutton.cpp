...
frmDeviceButton::frmDeviceButton(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::frmDeviceButton)
{
    ui->setupUi(this);
    this->initForm();
}
...
{
    //设置背景地图
    ui->labMap->setStyleSheet("border-image:url(:/images/bg_call.jpg);");
...
void frmDeviceButton::changeColor()
{
    ...
    DeviceButton::ButtonColor color = (DeviceButton::ButtonColor)index;
    btn1->setButtonColor(color);
    btn2->setButtonColor(color);
    btn3->setButtonColor(color);
}
