...
void SetWindowNewStyle(int w, int h) {
  ...
  SetWindowLong(GetHWnd(), GWL_STYLE,
                GetWindowLong(GetHWnd(), GWL_STYLE) & ~WS_CAPTION);
  // 初始化界面为圆形
...
void drawImg(IMAGE *pimg, int x, int y) {
  ...
  for (int iy = 0; iy < iheight; iy++) {
    for (int ix = 0; ix < iwidth; ix++) {
      byte a = (byte)(src[ix] >>
                      24); //计算透明通道的值[0,256) 0为完全透明 255为完全不透明
      if (a > 100) {
        dst[ix] = src[ix];
      }
    }
  ...
  if (MouseHit()) {
    ...
    } else if (msg.uMsg == WM_MOUSEMOVE) //鼠标移动
    {
      if (isMove) {
        ...
        /*把窗口移动到屏幕的x，y的位置
         * @hwnd:窗口句柄
         * @hwndInsertAfter:窗口的z顺序  HWND_TOPMOST {在前面,
         * 位于任何顶部窗口的前面}
         * @X，Y: 窗口左上角的新位置（相对于屏幕）
         * @cx，xy: 窗口大小
         * @uFlags:SWP_NOSIZE {忽略 cx、cy, 保持大小}
         */
        SetWindowPos(GetHWnd(), HWND_TOPMOST, CursorPos.x - WndPtSize.x,
                     CursorPos.y - WndPtSize.y, 0, 0, SWP_NOSIZE);
        // CursorPos.x - WndPtSize.x	//获取当前窗口左上角相对于屏幕的坐标
        // CursorPos.y - WndPtSize.y

        // printf("%d %d\n", CursorPos.x - WndPtSize.x, CursorPos.y -
        // WndPtSize.y);
      }
